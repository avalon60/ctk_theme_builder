...
class ControlPanel:
    ...
    def __init__(self):
        ...
        self._user_home_dir = Path(self._user_home_dir)
        self.process = None
        self.ctk_control_panel.protocol("WM_DELETE_WINDOW", self._close_panels)
        self._json_state = 'clean'
        self.widgets = {}
        self._rendered_harmony_buttons = []
        self._rendered_harmony_labels = []
        self._rendered_keystone_shades = []
        self.harmony_palette_running = False
        self._keystone_colour = None
        self.client_socket = None

        self._properties_view = 'All'
        self._widget_attributes = DEFAULT_VIEW_WIDGET_ATTRIBUTES

        # If this is the first time the app is run, create an app home directory.
        if not self._CONFIG_DIR.exists():
            print(f'Initialising application: {self._CONFIG_DIR}')
            os.mkdir(self._CONFIG_DIR)

        self._enable_tooltips = mod.preference_setting(db_file_path=DB_FILE_PATH, scope='user_preference',
                                                       preference_name='enable_tooltips')

        self._enable_palette_labels = mod.preference_setting(db_file_path=DB_FILE_PATH, scope='user_preference',
                                                             preference_name='enable_palette_labels')

        self._enable_single_click_paste = mod.preference_setting(db_file_path=DB_FILE_PATH, scope='user_preference',
        ...
        if self._last_theme_on_start == 'NO_DATA_FOUND':
            self._last_theme_on_start = mod.new_preference_dict(scope='user_preference',
                                                                preference_name='last_theme_on_start',
                                                                data_type='int', preference_value=0)
            mod.upsert_preference(db_file_path=DB_FILE_PATH, preference_row_dict=self._last_theme_on_start)
        if not self._theme_json_dir.exists():
            self._theme_json_dir = APP_HOME / 'user_themes'
        ...
        self._render_disabled = mod.preference_setting(db_file_path=DB_FILE_PATH, scope='auto_save',
                                                       preference_name='render_disabled')

        self._theme_author = mod.preference_setting(db_file_path=DB_FILE_PATH, scope='user_preference',
                                                    preference_name='theme_author')

        self._min_ctk_version = mod.preference_setting(db_file_path=DB_FILE_PATH, scope='system',
                                                       preference_name='min_ctk_version')

        base_ctk_version = ctk.__version__
        base_ctk_version = base_ctk_version[:base_ctk_version.index('.')]

        base_min_version = self._min_ctk_version
        ...
        self._control_panel_theme = str(APP_THEMES_DIR / control_panel_theme)
        # The control_panel_mode holds the  CustomTkinter appearance mode (Dark / Light)

        self._control_panel_mode = mod.preference_setting(db_file_path=DB_FILE_PATH,
                                                          scope='user_preference', preference_name='control_panel_mode')
        self._TEMP_DIR = TEMP_DIR

        ctl_mode_idx = cbtk.str_mode_to_int(self._control_panel_mode)
        ...
        self._ctl_frame_high = cbtk.theme_property_color(theme_file_path=control_panel_theme_path,
                                                         widget_type="CTkToplevel",
                                                         widget_property='fg_color',
                                                         mode=self._control_panel_mode
                                                         )

        self._ctl_text = cbtk.theme_property_color(theme_file_path=control_panel_theme_path,
                                                   widget_type="CTkLabel",
                                                   widget_property="text_color",
                                                   mode=self._control_panel_mode
                                                   )

        self._ctl_frame_border = cbtk.theme_property_color(theme_file_path=control_panel_theme_path,
        ...
        self._frm_control = ctk.CTkFrame(master=self.ctk_control_panel)
        self._frm_control.columnconfigure(1, weight=1)
        self._frm_control.pack(fill="both", expand=1)
        self._frm_control.rowconfigure(2, weight=1)
        self._frm_button = ctk.CTkFrame(master=self._frm_control)
        self._frm_geometry = ctk.CTkFrame(master=self._frm_control)
        self._frm_theme_palette = ctk.CTkFrame(master=self._frm_control)
        self._frm_colour_edit_widgets = ctk.CTkScrollableFrame(master=self._frm_control)

        button_frame = self._frm_button
        button_frame.grid(row=0, column=0, rowspan=3, columnspan=1, sticky='ns', padx=(5, 5), pady=(5, 5))

        self._frm_geometry.grid(row=0, column=1, rowspan=1, sticky='nswe', padx=(0, 5), pady=(5, 5))
        self._frm_theme_palette.grid(row=1, column=1, rowspan=1, sticky='nswe', padx=(0, 5), pady=(5, 5))
        self._frm_colour_edit_widgets.grid(row=2, column=1, rowspan=1, sticky='nswe', padx=(0, 5), pady=(5, 5))
        self._status_bar = cbtk.CBtkStatusBar(master=self.ctk_control_panel,
                                              status_text_life=30,
                                              use_grid=False)
        self.ctk_control_panel.bind("<Configure>", self._status_bar.auto_size_status_bar)
        ...
        self.seg_mode = ctk.CTkSegmentedButton(self._frm_button,
                                               values=['Light Mode', 'Dark Mode'],
                                               width=30,
                                               border_width=1,
                                               command=self._switch_preview_appearance_mode,
        ...
        self.swt_frame_mode = ctk.CTkSwitch(self._frm_button,
                                            state=tk.DISABLED,
                                            text='Top Frame',
        ...
        self._tk_render_disabled = tk.IntVar(master=self._frm_button, value=self._render_disabled)
        self._swt_render_disabled = ctk.CTkSwitch(master=self._frm_button,
                                                  text='Render Disabled',
                                                  variable=self._tk_render_disabled,
                                                  state=tk.DISABLED,
                                                  command=self._toggle_render_disabled)
        self._swt_render_disabled.grid(row=8, column=0, padx=10, pady=(10, 0))

        self.lbl_widget_view = ctk.CTkLabel(master=self._frm_button,
                                            text='Properties View:')
        self.lbl_widget_view.grid(row=9, column=0, sticky='w', pady=(20, 0), padx=(15, 10))

        self._widget_categories = all_widget_categories(DEFAULT_VIEW_WIDGET_ATTRIBUTES)
        self._views_list = self._view_list()
        self.tk_widget_view = tk.StringVar(value=DEFAULT_VIEW)
        self._opm_properties_view = ctk.CTkOptionMenu(master=self._frm_button,
                                                      command=self._update_properties_filter,
                                                      variable=self.tk_widget_view,
                                                      values=self._views_list,
                                                      state=tk.DISABLED)
        self._opm_properties_view.grid(row=10, column=0)
        if self._enable_tooltips:
            filter_view_tooltip = CTkToolTip(self._opm_properties_view,
                                             wraplength=400,
                                             justify="left",
                                             message='Views help you to target specific widgets, or groups of  '
                                                     'related widgets, and their associated properties. ')

        self.lbl_filter = ctk.CTkLabel(master=self._frm_button, text='Filter View:')
        self.lbl_filter.grid(row=11, column=0, sticky='w', pady=(10, 0), padx=(15, 10))

        self._widget_categories = all_widget_categories(DEFAULT_VIEW_WIDGET_ATTRIBUTES)
        self._opm_properties_filter = ctk.CTkOptionMenu(master=self._frm_button,
                                                        values=self._widget_categories,
                                                        command=self.set_filtered_widget_display,
                                                        state=tk.DISABLED)
        self._opm_properties_filter.grid(row=12, column=0)
        if self._enable_tooltips:
            filter_view_tooltip = CTkToolTip(self._opm_properties_filter,
                                             wraplength=400,
                                             justify="left",
                                             message='Within the properties available for the selected view, you can '
                                                     'elect to filter the display further.')

        self._btn_refresh = ctk.CTkButton(master=button_frame, text='Refresh Preview',
                                          command=self._reload_preview,
                                          state=tk.DISABLED)
        self._btn_refresh.grid(row=14, column=0, padx=5, pady=(30, 0))
        if self._enable_tooltips:
            sync_tooltip = CTkToolTip(self._btn_refresh,
                                      wraplength=250,
                                      justify="left",
        ...
        self._btn_reset.grid(row=15, column=0, padx=5, pady=(30, 5))

        self._btn_create = ctk.CTkButton(master=button_frame,
                                         text='New Theme',
                                         command=self._create_theme)
        self._btn_create.grid(row=16, column=0, padx=5, pady=(5, 5))

        self._btn_sync_modes = ctk.CTkButton(master=button_frame,
                                             text='Sync Modes',
                                             state=tk.DISABLED,
                                             command=self._sync_appearance_mode)
        self._btn_sync_modes.grid(row=17, column=0, padx=5, pady=(5, 5))
        if self._enable_tooltips:
            sync_tooltip = CTkToolTip(self._btn_sync_modes,
                                      wraplength=250,
                                      justify="left",
        ...
        self._btn_sync_palette = ctk.CTkButton(master=button_frame,
                                               text='Sync Palette',
                                               state=tk.DISABLED,
                                               command=self._sync_theme_palette)
        self._btn_sync_palette.grid(row=18, column=0, padx=5, pady=(5, 5))
        if self._enable_tooltips:
            sync_tooltip = CTkToolTip(self._btn_sync_modes,
                                      wraplength=250,
                                      justify="left",
        ...
        self._btn_save.grid(row=21, column=0, padx=5, pady=(30, 5))

        self._btn_save_as = ctk.CTkButton(master=button_frame,
                                          text='Save As',
                                          state=tk.DISABLED,
                                          command=self._save_theme_as)
        self._btn_save_as.grid(row=22, column=0, padx=5, pady=(5, 5))

        self._btn_delete = ctk.CTkButton(master=button_frame,
                                         text='Delete',
                                         state=tk.DISABLED,
                                         command=self._delete_theme)
        self._btn_delete.grid(row=23, column=0, padx=5, pady=(5, 5))
        ...
        if int(base_min_version) > int(base_ctk_version):
            print(f'WARNING: The version of CustomTkinter, on your system, is incompatible. Please upgrade to '
                  f'CustomTkinter {self._min_ctk_version} or later.')
            confirm = CTkMessagebox(master=self.ctk_control_panel,
                                    ...
                                    message=f'The version of CustomTkinter, on your system, is incompatible. '
                                            f'Please upgrade to CustomTkinter {self._min_ctk_version} or later.',
                                    option_1='OK')
            if confirm.get() == 'OK':
                exit(1)
    ...
    def _set_widget_colour(self, widget_property, new_colour):
        ...
        if not cbtk.valid_colour(new_colour):
            self._status_bar.set_status_text(status_text=f'Paste action ignored - not a valid colour code.')
            return
        ...
        appearance_mode_index = cbtk.str_mode_to_int(self._appearance_mode)
        ...
        self._theme_json_data[json_widget_type][split_property][appearance_mode_index] = new_colour
        parameters = []
    ...
    def _create_theme_palette(self, theme_name):
        self._theme_palette = mod.json_dict(json_file_path=ETC_DIR / 'default_palette.json')
        palette_json = theme_name.replace('.json', '') + '.json'
        self._palette_file = self._palettes_dir / palette_json
        with open(self._palette_file, "w") as f:
            json.dump(self._theme_palette, f, indent=2)
    ...
    def _create_menu(self):
        ...
        self.file_menu.add_command(label='Refresh Preview', command=self._reload_preview)
        self.file_menu.add_separator()
        self.file_menu.add_command(label='Reset', command=self._reset_theme)
        self.file_menu.add_command(label='New Theme', command=self._create_theme)
        self.file_menu.add_command(label='Sync Modes', command=self._sync_appearance_mode)
        self.file_menu.add_command(label='Sync Palette', command=self._sync_theme_palette)
        self.file_menu.add_separator()
        self.file_menu.add_command(label='Save', command=self._save_theme)
        self.file_menu.add_command(label='Save As', command=self._save_theme_as)
        ...
        self.tools_menu.add_command(label='Preferences', command=self._launch_preferences_dialog)
        self.tools_menu.add_command(label='Colour Harmonics', command=self._launch_harmony_dialog, state=tk.DISABLED)
        self.tools_menu.add_command(label='About', command=self._about)

        self._set_option_states()

    def _copy_property_colour(self, event=None, widget_property=None):
        colour = None
        try:
    ...
    def _launch_preferences_dialog(self):
        self._top_prefs = ctk.CTkToplevel(self.ctk_control_panel)
        self._top_prefs.title('CTk Theme Builder Preferences')
        self._top_prefs.geometry('500x550')
        ...
        if not self._theme_author:
            self._theme_author = self.user_name
        self.tk_author_name = tk.StringVar(value=self._theme_author)
        ...
        self._opm_control_panel_theme = ctk.CTkOptionMenu(master=frm_widgets,
                                                          ...
                                                          values=self._app_themes_list())
        self._opm_control_panel_theme.grid(row=widget_start_row, column=1, padx=0, pady=5, sticky='w')
        widget_start_row += 1
        ...
        if self._control_panel_mode == 'Dark':
            rdo_light.deselect()
            rdo_dark.select()
        elif self._control_panel_mode == 'Light':
            rdo_dark.deselect()
            rdo_light.select()
        ...
        self._swt_enable_tooltips.grid(row=widget_start_row, column=1, padx=(0, 0), pady=10, sticky='w')
        widget_start_row += 1

        self._tk_enable_palette_labels = tk.IntVar(master=frm_widgets)
        self._tk_enable_palette_labels.set(self._enable_palette_labels)
        self._swt_enable_palette_labels = ctk.CTkSwitch(master=frm_widgets,
                                                        text='Colour Palette Labels',
                                                        variable=self._tk_enable_palette_labels,
                                                        command=self._get_palette_label_setting)
        self._swt_enable_palette_labels.grid(row=widget_start_row, column=1, padx=(0, 0), pady=10, sticky='w')

        if self._enable_tooltips:
            btn_enable_palette_labels_tooltip = CTkToolTip(self._swt_enable_palette_labels,
                                                           "Enable/disable the rendering of the colour palette "
                                                           "labels.")

        widget_start_row += 1
        self._tk_last_theme_on_start = tk.IntVar(master=frm_widgets)
        self._tk_last_theme_on_start.set(self._last_theme_on_start)
        self._swt_last_theme_on_start = ctk.CTkSwitch(master=frm_widgets,
                                                      text='Load Last Theme',
                                                      variable=self._tk_last_theme_on_start,
                                                      command=self._get_last_theme_on_start)
        self._swt_last_theme_on_start.grid(row=widget_start_row, column=1, padx=(0, 0), pady=10, sticky='w')
        if self._enable_tooltips:
            last_theme_on_start_tooltip = CTkToolTip(self._swt_last_theme_on_start,
                                                     wraplength=250,
                                                     justify="left",
        ...
        widget_start_row += 1
        self._tk_enable_single_click_paste = tk.IntVar(master=frm_widgets)
        self._tk_enable_single_click_paste.set(self._enable_single_click_paste)
        self._swt_enable_single_click_paste = ctk.CTkSwitch(master=frm_widgets,
                                                            text='Single Click Paste',
                                                            variable=self._tk_enable_single_click_paste,
                                                            command=self._get_single_click_paste_setting)
        self._swt_enable_single_click_paste.grid(row=widget_start_row, column=1, padx=(0, 0), pady=10, sticky='w')

        widget_start_row += 1
        if self._enable_tooltips:
            btn_enable_tooltips_tooltip = CTkToolTip(self._swt_enable_single_click_paste,
                                                     wraplength=400,
        ...
        self._folder_image = cbtk.load_image(light_image=APP_IMAGES / 'folder.png', image_size=(20, 20))
        ...
        self._lbl_pref_theme_dir_disp = ctk.CTkLabel(master=frm_widgets, text=self._theme_json_dir, justify="right",
                                                     font=mod.SMALL_TEXT)
        self._lbl_pref_theme_dir_disp.grid(row=widget_start_row, column=1, padx=5, pady=5, sticky='e')
        widget_start_row += 1
    ...
    def _save_theme_palette(self, theme_name=None):
        ...
        if theme_name is None:
            palette_json = self._palette_file
        else:
            palette_json = theme_name.replace('.json', '') + '.json'

        # Update the button colour settings to the self._theme_palette dictionary
        self._palette_file = self._palettes_dir / palette_json
        if self._appearance_mode.lower() == 'light':
            mode = 0
        else:
            mode = 1
        for entry_id, button in enumerate(self._theme_palette_tiles):
            fg_colour = self._theme_palette_tiles[entry_id].cget('fg_color')
            self._theme_palette[str(entry_id)][mode] = fg_colour

        with open(self._palette_file, "w") as f:
            json.dump(self._theme_palette, f, indent=2)

    def _send_command_json(self, command_type: str, command: str, parameters: list = None):
        ...
        self._send_message(message=message_json_str)
    ...
    def _send_message(self, message):
        ...
        send_length, message = self._prepare_message(message)
        ...
        send_length, message = self._prepare_message(DISCONNECT_JSON)
        client.send(send_length)
        client.send(message)
        client.close()

    def _render_geometry_buttons(self):
        ...
        lbl_header = ctk.CTkLabel(master=self._frm_geometry,
        ...
        row = 1
        column = 0
        btn_geo_button = ctk.CTkButton(master=self._frm_geometry,
                                       text='Button',
                                       height=button_height,
                                       width=button_width,
                                       corner_radius=corner_radius,
                                       border_width=border_width,
                                       command=lambda: self._launch_widget_geometry('CTkButton'))
        ...
        btn_geo_checkbox = ctk.CTkButton(master=self._frm_geometry,
                                         text='Checkbox',
                                         height=button_height,
                                         width=button_width,
                                         corner_radius=corner_radius,
                                         border_width=border_width,
                                         command=lambda: self._launch_widget_geometry('CTkCheckBox'))
        ...
        btn_geo_combobox = ctk.CTkButton(master=self._frm_geometry,
                                         text='ComboBox',
                                         height=button_height,
                                         width=button_width,
                                         corner_radius=corner_radius,
                                         border_width=border_width,
                                         command=lambda: self._launch_widget_geometry('CTkComboBox'))
        ...
        btn_geo_entry = ctk.CTkButton(master=self._frm_geometry,
                                      text='Entry',
                                      height=button_height,
                                      width=button_width,
                                      corner_radius=corner_radius,
                                      border_width=border_width,
                                      command=lambda: self._launch_widget_geometry('CTkEntry'))
        ...
        btn_geo_frame = ctk.CTkButton(master=self._frm_geometry,
                                      text='Frame',
                                      height=button_height,
                                      width=button_width,
                                      corner_radius=corner_radius,
                                      border_width=border_width,
                                      command=lambda: self._launch_widget_geometry('CTkFrame'))
        ...
        btn_geo_label = ctk.CTkButton(master=self._frm_geometry,
                                      text='Label',
                                      height=button_height,
                                      width=button_width,
                                      corner_radius=corner_radius,
                                      border_width=border_width,
                                      command=lambda: self._launch_widget_geometry('CTkLabel'))
        ...
        column += 1
        btn_geo_option_menu = ctk.CTkButton(master=self._frm_geometry,
                                            text='OptionMenu',
                                            height=button_height,
                                            width=button_width,
                                            corner_radius=corner_radius,
                                            border_width=border_width,
                                            command=lambda: self._launch_widget_geometry('CTkOptionMenu'))
        ...
        btn_geo_progress = ctk.CTkButton(master=self._frm_geometry,
                                         text='ProgressBar',
                                         height=button_height,
                                         width=button_width,
                                         corner_radius=corner_radius,
                                         border_width=border_width,
                                         command=lambda: self._launch_widget_geometry('CTkProgressBar'))
        ...
        btn_geo_radio = ctk.CTkButton(master=self._frm_geometry,
                                      text='RadioButton',
                                      height=button_height,
                                      width=button_width,
                                      corner_radius=corner_radius,
                                      border_width=border_width,
                                      command=lambda: self._launch_widget_geometry('CTkRadioButton'))
        ...
        btn_geo_scrollbar = ctk.CTkButton(master=self._frm_geometry,
                                          text='SegButton',
                                          height=button_height,
                                          width=button_width,
                                          corner_radius=corner_radius,
                                          border_width=border_width,
                                          command=lambda: self._launch_widget_geometry('CTkSegmentedButton'))
        ...
        btn_geo_scrollbar = ctk.CTkButton(master=self._frm_geometry,
                                          ...
                                          corner_radius=corner_radius,
                                          border_width=border_width,
                                          command=lambda: self._launch_widget_geometry('CTkScrollbar'))
        btn_geo_scrollbar.grid(row=row,
                               column=column,
                               padx=pad_x,
                               pady=pad_y)
        ...
        btn_geo_slider = ctk.CTkButton(master=self._frm_geometry,
                                       text='Slider',
                                       height=button_height,
                                       width=button_width,
                                       corner_radius=corner_radius,
                                       border_width=border_width,
                                       command=lambda: self._launch_widget_geometry('CTkSlider'))
        ...
        btn_geo_switch = ctk.CTkButton(master=self._frm_geometry,
                                       text='Switch',
                                       height=button_height,
                                       width=button_width,
                                       corner_radius=corner_radius,
                                       border_width=border_width,
                                       command=lambda: self._launch_widget_geometry('CTkSwitch'))
        ...
        btn_geo_switch = ctk.CTkButton(master=self._frm_geometry,
                                       ...
                                       corner_radius=corner_radius,
                                       border_width=border_width,
                                       command=lambda: self._launch_widget_geometry('CTkTextbox'))
        btn_geo_switch.grid(row=row,
                            column=column,
                            padx=pad_x,
                            pady=pad_y)
    ...
    def _launch_widget_geometry(self, widget_type):
        def slider_callback(property_name, value):
            ...
            self._geometry_edit_values[property_name] = property_value
        ...
        self._geometry_edit_values = {}
        preview_frame_top = self._theme_json_data['CTkFrame']['top_fg_color'][
            cbtk.str_mode_to_int(self._appearance_mode)]

        self._top_geometry = ctk.CTkToplevel(self.ctk_control_panel)
        self._top_geometry.title(f'{widget_type} Widget Geometry')

        self._restore_geom_geometry()

        # Make preferences dialog modal
        self._top_geometry.rowconfigure(0, weight=1)
        self._top_geometry.rowconfigure(1, weight=0)
        # self._top_geometry.columnconfigure(0, weight=0)
        # self._top_geometry.columnconfigure(1, weight=1)

        preview_text_colour = self._theme_json_data['CTkLabel']['text_color'][
            cbtk.str_mode_to_int(self._appearance_mode)]

        frm_main = ctk.CTkFrame(master=self._top_geometry, corner_radius=5)
        frm_main.grid(column=0, row=0, sticky='nsew')
        frm_main.columnconfigure(0, weight=0)
        frm_main.columnconfigure((1, 2), weight=1)
        frm_main.rowconfigure(0, weight=1)

        frame_fg_color = self._theme_json_data['CTkFrame']['fg_color'][
            cbtk.str_mode_to_int(self._appearance_mode)]
        json_widget_type = mod.json_widget_type(widget_type=widget_type)

        mode = cbtk.str_mode_to_int(self._appearance_mode)
        if widget_type == 'CTkFrame':
            self._top_geometry.geometry('764x280')
            frm_widget_preview_low = ctk.CTkFrame(master=frm_main,
                                                  fg_color=cbtk.contrast_colour(preview_frame_top, 20)
                                                  )
            frm_main.configure(corner_radius=self._theme_json_data[widget_type]['corner_radius'])
        else:
            frm_widget_preview_low = ctk.CTkFrame(master=frm_main,
                                                  corner_radius=5,
        ...
        btn_save = ctk.CTkButton(master=frm_buttons, text='Save',
                                 command=lambda w_type=widget_type: self._save_geometry_edits(widget_type=w_type))
        ...
        if widget_type == 'CTkButton':
            self._top_geometry.geometry('764x234')
            button_text_colour = self._theme_json_data[json_widget_type]['text_color'][mode]
            button_fg_colour = self._theme_json_data[json_widget_type]['fg_color'][mode]
            button_hover_colour = self._theme_json_data[json_widget_type]['hover_color'][mode]
            button_border_colour = self._theme_json_data[json_widget_type]['border_color'][mode]
            ...
            geometry_widget = ctk.CTkButton(master=frm_widget_preview_low,
                                            ...
                                            text='CTkButton',
                                            corner_radius=self._theme_json_data['CTkButton']['corner_radius'],
                                            border_width=self._theme_json_data['CTkButton']['border_width'])
        elif widget_type == 'CTkCheckBox':
            self._top_geometry.geometry('786x232')
            checkbox_fg_color = self._theme_json_data['CTkCheckbox']['fg_color'][mode]

            checkbox_border_color = self._theme_json_data['CTkCheckbox']['border_color'][mode]

            checkbox_hover_color = self._theme_json_data['CTkCheckbox']['hover_color'][mode]

            checkbox_checkmark_color = self._theme_json_data['CTkCheckbox']['checkmark_color'][mode]

            checkbox_text_color = self._theme_json_data['CTkCheckbox']['text_color'][mode]
            ...
            geometry_widget = ctk.CTkCheckBox(master=frm_widget_preview_low,
                                              ...
                                              corner_radius=self._theme_json_data['CTkCheckbox']['corner_radius'],
                                              border_width=self._theme_json_data['CTkCheckbox']['border_width'])
        elif widget_type == 'CTkComboBox':
            self._top_geometry.geometry('795x234')

            combobox_fg_color = self._theme_json_data['CTkComboBox']['fg_color'][mode]

            combobox_text_color = self._theme_json_data['CTkComboBox']['text_color'][mode]

            combobox_border_color = self._theme_json_data['CTkComboBox']['border_color'][mode]

            combobox_button_colour = self._theme_json_data[json_widget_type]['button_color'][mode]

            combobox_button_hover_colour = self._theme_json_data[json_widget_type]['button_hover_color'][mode]

            dropdown_fg_colour = self._theme_json_data['DropdownMenu']['fg_color'][mode]

            dropdown_hover_colour = self._theme_json_data['DropdownMenu']['hover_color'][mode]

            dropdown_text_colour = self._theme_json_data['DropdownMenu']['text_color'][mode]
            ...
            geometry_widget = ctk.CTkFrame(master=frm_widget_preview_low, fg_color=preview_frame_top,
                                           corner_radius=self._theme_json_data['CTkFrame']['corner_radius'],
                                           border_width=self._theme_json_data['CTkFrame']['border_width'])
            ...
            self._top_geometry.geometry('755x160')
            geometry_widget = ctk.CTkLabel(master=frm_widget_preview_low,
                                           ...
                                           corner_radius=self._theme_json_data[widget_type]['corner_radius'])
            ...
            fg_color = self._theme_json_data['CTkEntry']['fg_color'][mode]
            border_color = self._theme_json_data['CTkEntry']['border_color'][mode]
            self._top_geometry.geometry('754x235')
            ...
            self._top_geometry.geometry('807x225')
            progressbar_fg_color = self._theme_json_data['CTkProgressBar']['fg_color'][mode]

            progressbar_progress_color = self._theme_json_data['CTkProgressBar']['progress_color'][mode]

            progressbar_border_color = self._theme_json_data['CTkProgressBar']['border_color'][mode]
            geometry_widget = ctk.CTkProgressBar(master=frm_widget_preview_low,
                                                 ...
                                                 corner_radius=self._theme_json_data[json_widget_type]
                                                 ['corner_radius'],
                                                 border_width=self._theme_json_data[json_widget_type]
                                                 ['border_width'])
        elif widget_type == 'CTkSlider':
            self._top_geometry.geometry('760x301')
            geometry_widget = ctk.CTkSlider(master=frm_widget_preview_low,
                                            border_width=self._theme_json_data[widget_type]['border_width'])

        elif widget_type == 'CTkOptionMenu':
            self._top_geometry.geometry('806x161')

            optionmenu_fg_colour = self._theme_json_data[json_widget_type]['fg_color'][mode]

            optionmenu_button_colour = self._theme_json_data[json_widget_type]['button_color'][mode]

            optionmenu_button_hover_colour = self._theme_json_data[json_widget_type]['button_hover_color'][mode]

            optionmenu_text_color = self._theme_json_data[json_widget_type]['text_color'][mode]
            ...
            geometry_widget = ctk.CTkOptionMenu(master=frm_widget_preview_low,
                                                ...
                                                dropdown_text_color=dropdown_text_colour,
                                                dropdown_hover_color=dropdown_hover_colour,
                                                corner_radius=self._theme_json_data['CTkOptionMenu']['corner_radius'],
                                                values=["Option 1", "Option 2", "Option 3..."])
            geometry_widget.set("CTkOptionMenu")


        elif widget_type == 'CTkRadioButton':
            self._top_geometry.geometry('800x301')
            radiobutton_fg_color = self._theme_json_data['CTkRadiobutton']['fg_color'][mode]

            radiobutton_border_color = self._theme_json_data['CTkRadiobutton']['border_color'][mode]

            radiobutton_hover_color = self._theme_json_data['CTkRadiobutton']['hover_color'][mode]

            radiobutton_text_color = self._theme_json_data['CTkRadiobutton']['text_color'][mode]

            label_text_colour = self._theme_json_data['CTkLabel']['text_color'][mode]
            ...
            geometry_widget = ctk.CTkRadioButton(master=frm_widget_preview_low,
                                                 ...
                                                 corner_radius=self._theme_json_data[json_widget_type]
                                                 ['corner_radius'],
                                                 border_width_checked=self._theme_json_data[json_widget_type]
                                                 ['border_width_checked'],
                                                 border_width_unchecked=self._theme_json_data[json_widget_type]
            ...
            self._top_geometry.geometry('847x232')
            # CTkTextbox
            seg_fg_color = self._theme_json_data[json_widget_type]['fg_color'][mode]
            print(f'DEBUG: applying colour: {seg_fg_color}')
            seg_selected_color = self._theme_json_data[json_widget_type]['selected_color'][mode]
            seg_selected_hover_color = self._theme_json_data[json_widget_type]['selected_hover_color'][mode]
            seg_unselected_color = self._theme_json_data[json_widget_type]['unselected_color'][mode]
            seg_unselected_hover_color = self._theme_json_data[json_widget_type]['unselected_hover_color'][mode]
            seg_text_color = self._theme_json_data[json_widget_type]['text_color'][mode]
            seg_text_color_disabled = self._theme_json_data[json_widget_type]['text_color_disabled'][mode]
            ...
            self._top_geometry.geometry('766x299')
            switch_fg_colour = self._theme_json_data[json_widget_type]['fg_color'][mode]

            switch_button_colour = self._theme_json_data[json_widget_type]['button_color'][mode]

            switch_button_hover_colour = self._theme_json_data[json_widget_type]['button_hover_color'][mode]

            switch_progress_colour = self._theme_json_data[json_widget_type]['progress_color'][mode]

            switch_text_colour = self._theme_json_data[json_widget_type]['text_color'][mode]
            ...
            geometry_widget = ctk.CTkSwitch(master=frm_widget_preview_low,
                                            ...
                                            corner_radius=self._theme_json_data[json_widget_type]['corner_radius'],
                                            border_width=self._theme_json_data[json_widget_type]['border_width']
                                            )
        elif widget_type == 'CTkScrollbar':
            self._top_geometry.geometry('782x267')
            # Harness the scrollbar incorporated
            # to the CTkScrollableFrame widget.
            self._top_geometry.geometry('800x280')
            scrollbar_fg_color = self._theme_json_data[json_widget_type]['fg_color']
            if not isinstance(scrollbar_fg_color, str):
                scrollbar_fg_color = scrollbar_fg_color[cbtk.str_mode_to_int(self._appearance_mode)]
            elif scrollbar_fg_color == "transparent":
                scrollbar_fg_color = self._theme_json_data['CTkFrame']['fg_color'][
                    cbtk.str_mode_to_int(self._appearance_mode)]
            scrollbar_button_color = self._theme_json_data[json_widget_type]['button_color']
            scrollbar_button_hover_color = self._theme_json_data[json_widget_type]['button_hover_color']

            frm_preview = ctk.CTkFrame(frm_widget_preview_low, corner_radius=0)
            frm_preview.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

            # create textbox
            textbox_fg_color = self._theme_json_data['CTkTextbox']['fg_color'][
                cbtk.str_mode_to_int(self._appearance_mode)]
            if not isinstance(textbox_fg_color, str):
                textbox_fg_color = textbox_fg_color[cbtk.str_mode_to_int(self._appearance_mode)]
            textbox_border_color = self._theme_json_data['CTkTextbox']['border_color'][
                cbtk.str_mode_to_int(self._appearance_mode)]
            textbox_text_color = self._theme_json_data['CTkTextbox']['text_color'][
                cbtk.str_mode_to_int(self._appearance_mode)]
            ...
            self._top_geometry.geometry('776x243')
            # CTkTextbox
            textbox_fg_color = self._theme_json_data[json_widget_type]['fg_color'][mode]
            if not isinstance(textbox_fg_color, str):
                textbox_fg_color = textbox_fg_color[cbtk.str_mode_to_int(self._appearance_mode)][mode]
            elif textbox_fg_color == "transparent":
                textbox_fg_color = self._theme_json_data['CTkFrame']['fg_color'][mode]
            textbox_border_color = self._theme_json_data[json_widget_type]['border_color'][mode]
            textbox_button_color = self._theme_json_data[json_widget_type]['scrollbar_button_color'][mode]
            textbox_button_hover_color = self._theme_json_data[json_widget_type]['scrollbar_button_hover_color'][mode]
            textbox_text_color = self._theme_json_data[json_widget_type]['text_color'][mode]
        ...
        for property, parameters in widget_parameter.items():
            ...
            current_value = int(self._theme_json_data[json_widget_type][property])
        ...
        self._top_geometry.wait_window()

    def close_geometry_dialog(self):
        self._save_widget_geom_geometry()
        self._top_geometry.destroy()

    def _save_geometry_edits(self, widget_type):
        for widget_property, property_value in self._geometry_edit_values.items():
            parameters = []
            # This function call is necessary, because there are several naming
            # inconsistencies (at least in CTk 5.1.2), between widget names.
            json_widget_type = mod.json_widget_type(widget_type=widget_type)
            if self._theme_json_data[json_widget_type][widget_property] != property_value:
                self._theme_json_data[json_widget_type][widget_property] = property_value
                self._json_state = 'dirty'
                ...
                with open(self._wip_json, "w") as f:
                    json.dump(self._theme_json_data, f, indent=2)
                self._send_command_json(command_type='geometry',
    ...
    def _set_option_states(self):
        ...
        if self._json_state == 'dirty' and self._theme:
            self.file_menu.entryconfig('Reset', state=tk_state)
            self.file_menu.entryconfig('Save', state=tk_state)
            self._btn_reset.configure(state=tk_state)
            self._btn_save.configure(state=tk_state)
        else:
        ...
        self._opm_properties_filter.configure(state=tk_state)
        self._opm_properties_view.configure(state=tk_state)
        self._swt_render_disabled.configure(state=tk_state)
        self.swt_frame_mode.configure(state=tk_state)
        self._btn_sync_modes.configure(state=tk_state)
        self._btn_sync_palette.configure(state=tk_state)
        self._btn_refresh.configure(state=tk_state)

        if self.harmony_palette_running:
            self.tools_menu.entryconfig('Colour Harmonics', state=tk.DISABLED)
        elif not self.harmony_palette_running and self._theme:
    ...
    def _save_preferences(self):
        ...
        if self._new_theme_json_dir is not None:
            ...
            if not mod.update_preference_value(db_file_path=DB_FILE_PATH, scope='user_preference',
                                               preference_name='theme_json_dir',
                                               preference_value=str(self._theme_json_dir)):
        ...
        if not mod.update_preference_value(db_file_path=DB_FILE_PATH, scope='user_preference',
                                           ...
                                           preference_value=self._opm_control_panel_theme.get()):
            print(f'Row miss updating preferences control panel theme.')

        control_panel_mode = self._tk_appearance_mode_var.get()
        if not mod.update_preference_value(db_file_path=DB_FILE_PATH, scope='user_preference',
        ...
        if not mod.update_preference_value(db_file_path=DB_FILE_PATH, scope='user_preference',
                                           preference_name='enable_tooltips',
                                           preference_value=self._enable_tooltips):
            print(f'Row miss updating preferences control panel appearance mode.')

        if not mod.update_preference_value(db_file_path=DB_FILE_PATH, scope='user_preference',
                                           preference_name='last_theme_on_start',
                                           preference_value=self._last_theme_on_start):
            print(f'Row miss updating preferences control panel last theme on start.')

        if not mod.update_preference_value(db_file_path=DB_FILE_PATH, scope='user_preference',
                                           preference_name='enable_palette_labels',
                                           preference_value=self._enable_palette_labels):
            print(f'Row miss updating preferences: enable palette labels.')

        if not mod.update_preference_value(db_file_path=DB_FILE_PATH, scope='user_preference',
                                           preference_name='enable_single_click_paste',
                                           preference_value=self._enable_single_click_paste):
        ...
        self._control_panel_mode = control_panel_mode
        self._tk_appearance_mode_var.set(self._control_panel_mode)
        self._status_bar.set_status_text(status_text=f'Preferences saved.')
        self._top_prefs.destroy()

    def _switch_preview_appearance_mode(self, event='event'):
        ...
        if not mod.update_preference_value(db_file_path=DB_FILE_PATH, scope='auto_save',
                                           preference_name='appearance_mode',
                                           preference_value=self._appearance_mode):
        ...
        self._load_theme_palette()
        self.render_widget_properties()

        self._send_command_json(command_type='program',
                                command='set_appearance_mode',
                                parameters=[self._appearance_mode])

        # self._load_theme()

    def _render_theme_palette(self):
        render_labels = self._enable_palette_labels
        palette_entries = mod.colour_palette_entries(db_file_path=DB_FILE_PATH)
        menus = []
        self._theme_palette_tiles = []
        ...
        if preview_appearance_mode == 'Light Mode':
            ...
            self._appearance_mode = 'Dark'
        self.lbl_palette_header = ctk.CTkLabel(master=self._frm_theme_palette,
        ...
        for tile_dict in palette_entries:
            ...
            lbl_tile_label = ctk.CTkLabel(master=self._frm_theme_palette,
            ...
            btn_colour_tile = ctk.CTkButton(master=self._frm_theme_palette,
                                            border_width=2,
                                            width=90,
                                            height=40,
                                            corner_radius=10,
            ...
            self._theme_palette_tiles.append(btn_colour_tile)
            # colour_tile = self._theme_palette_tiles[entry_id]
            ...
            context_menu.add_command(label="Copy",
                                     ...
                                     self._copy_palette_colour(palette_button_id=button_id))

            context_menu.add_command(label="Paste",
                                     command=lambda button_id=entry_id:
                                     self._paste_palette_colour(event=None, palette_button_id=button_id))
            ...
            context_menu.add_command(label="Colour Picker",
                                     command=lambda button_id=entry_id: self._colour_picker(button_id))
            menus.append(context_menu)

            if self._enable_single_click_paste:
                btn_colour_tile.bind("<Button-1>",
                                     lambda event, button_id=entry_id: self._paste_palette_colour(event,
                                                                                                  button_id))

            btn_colour_tile.bind("<Button-3>",
                                 lambda event, menu=menus[entry_id], button_id=entry_id: self._context_menu(event,
                                                                                                            menu))
    ...
    def _toggle_frame_mode(self):
        ...
        if frame_mode == 'top':
            self._send_command_json(command_type='program', command='render_top_frame')
        else:
            self._send_command_json(command_type='program', command='render_base_frame')

    def _toggle_render_disabled(self):
        render_state = self._swt_render_disabled.get()
        if render_state:
            self._send_command_json(command_type='program', command='render_preview_disabled')
        else:
            self._send_command_json(command_type='program', command='render_preview_enabled')
    ...
    def _update_properties_filter(self, view_name):
        ...
        self._widget_attributes = mod.json_dict(json_file_path=view_file)
        ...
        self._opm_properties_filter.set('All')
        self.set_filtered_widget_display()
        # self._send_preview_command('Update properties filter...')
    ...
    def _user_themes_list(self):
        ...
        json_files = list(self._theme_json_dir.glob('*.json'))
        theme_names = []
        for file in json_files:
            file = os.path.basename(file)
            theme_name = os.path.splitext(file)[0]
    ...
    def _load_theme(self, event=None, reload_preview: bool = True):
        ...
        self._theme_file = selected_theme + '.json'
        self._source_json_file = self._theme_json_dir / self._theme_file
        if not self._source_json_file.exists():
        ...
        if self._source_json_file:
            ...
            self._preview_json = self._TEMP_DIR / self._theme_file
            shutil.copyfile(self._source_json_file, self._wip_json)
            self._theme_json_data = mod.json_dict(json_file_path=self._wip_json)

            # self._update_config(section='preferences', option='theme_json_dir', value=self._theme_json_dir)
            self._render_geometry_buttons()
            self._render_theme_palette()
            self._load_theme_palette()
            self.set_filtered_widget_display()
            ...
            self._btn_sync_modes.configure(state=tk.NORMAL)
            self._opm_properties_view.configure(state=tk.NORMAL)
            self._swt_render_disabled.configure(state=tk.NORMAL)
            ...
            palette_file = self._palettes_dir / palette_file
            if not palette_file.exists():
                self._create_theme_palette(selected_theme)
    ...
    def _save_harmonics_geometry(self):
        ...
        panel_geometry = self._top_harmony.geometry()
    ...
    def _save_widget_geom_geometry(self):
        ...
        panel_geometry = self._top_geometry.geometry()
        geometry_row["preference_value"] = panel_geometry
        mod.upsert_preference(db_file_path=DB_FILE_PATH, preference_row_dict=geometry_row)

    def _on_harmonic_close(self):
        self._rendered_keystone_shades = []
        self._save_harmonics_geometry()
        self._top_harmony.destroy()
        self.harmony_palette_running = False
        self._set_option_states()

    def _restore_geom_geometry(self):
        ...
        saved_geometry = mod.preference_setting(db_file_path=DB_FILE_PATH,
                                                scope='window_geometry',
                                                preference_name='widget_geometry')
        self._top_geometry.geometry(saved_geometry)
        # self._top_geometry.resizable(False, False)

    def _restore_harmony_geometry(self):
        ...
        self._top_harmony.geometry(saved_geometry)
        self._top_harmony.resizable(False, False)

    def _launch_harmony_dialog(self):
        ...
        self._top_harmony = tk.Toplevel(master=self.ctk_control_panel)
        self._top_harmony.title('Colour Harmonics')

        self._top_harmony.columnconfigure(0, weight=1)
        self._top_harmony.rowconfigure(0, weight=1)
        self._restore_harmony_geometry()

        frm_main = ctk.CTkFrame(master=self._top_harmony, corner_radius=0)
        frm_main.grid(column=0, row=0, sticky='nsew')
        frm_main.columnconfigure(0, weight=1)
        frm_main.rowconfigure(0, weight=1)
        ...
        self._frm_harmony_colours = ctk.CTkFrame(master=frm_main)
        self._frm_harmony_colours.grid(column=0, row=1, padx=10, pady=10, sticky='nsew')

        self._frm_shades_palette = ctk.CTkFrame(master=frm_main)
        self._frm_shades_palette.grid(column=2, row=0, padx=(0, 10), pady=10, sticky='nsew', rowspan=2)

        frm_buttons = ctk.CTkFrame(master=frm_main)
        frm_buttons.grid(column=0, row=2, padx=10, pady=(5, 0), sticky='ew', columnspan=3)

        self._harmony_status_bar = cbtk.CBtkStatusBar(master=self._top_harmony,
                                                      status_text_life=30,
                                                      use_grid=True)
        ...
        lbl_harmony_header = ctk.CTkLabel(master=self._frm_harmony_colours,
        ...
        bg_colour = self._theme_json_data.get('provenance', {}).get('keystone colour', None)
        self._btn_keystone_colour = ctk.CTkButton(master=frm_controls,
                                                  border_width=3,
                                                  fg_color=bg_colour,
                                                  width=120,
                                                  height=90,
        ...
        self._btn_keystone_colour.grid(row=2, column=0, padx=5, pady=(0, 10))
        ...
        self.opm_harmony_method = ctk.CTkOptionMenu(master=frm_controls, values=['Analogous',
                                                                                 ...
                                                                                 'Triadic',
                                                                                 'Tetradic'],
                                                    command=self._switch_harmony_method)
        self.opm_harmony_method.grid(row=3, column=0, padx=(10, 10), pady=(0, 10))
        harmony_method = self._theme_json_data.get('provenance', {}).get('harmony method', 'Analogous')

        self.opm_harmony_method.set(harmony_method)
        # self.opm_harmony_method.set('Analogous')

        btn_palette_tooltip = cbtk.CBtkToolTip(self._btn_keystone_colour,
                                               text='Right click for options.')
        mnu_keystone = cbtk.CBtkMenu(self._btn_keystone_colour, tearoff=False)
        mnu_keystone.add_command(label="Copy",
                                 command=self._copy_harmony_input_colour)

        mnu_keystone.add_command(label="Paste",
                                 command=self._paste_harmony_keystone_colour)

        mnu_keystone.add_command(label="Colour Picker",
                                 command=self._harmony_input_colour_picker)
        # if self._enable_single_click_paste:
        #    self._btn_keystone_colour.bind("<Button-1>",
        #                     lambda event, button_id=self._paste_harmony_keystone_colour)

        self._btn_keystone_colour.bind("<Button-3>",
                                       lambda event, menu=mnu_keystone: self._context_menu(event, menu))
        ...
        btn_close = ctk.CTkButton(master=frm_buttons,
                                  text='Close',
                                  command=self._on_harmonic_close)
        ...
        harmony_method = self._theme_json_data.get('provenance', {}).get('harmony method', None)
        self.set_harmony_keystone(colour_code=bg_colour, method=harmony_method)
        self._switch_harmony_method()
        self.harmony_palette_running = True
        self._set_option_states()
        self._top_harmony.protocol("WM_DELETE_WINDOW", self._on_harmonic_close)
        self._top_harmony.grab_set()

    def _switch_harmony_method(self, event='event'):
        ...
        for button in self._rendered_harmony_buttons:
            button.destroy()
            self._rendered_harmony_buttons = []
        ...
        if harmony_entries == 1:
            self._top_harmony.geometry(f"510x{self.HARMONICS_HEIGHT1}")
        elif harmony_entries == 2:
            self._top_harmony.geometry(f"650x{self.HARMONICS_HEIGHT2}")
        elif harmony_entries == 3:
            self._top_harmony.geometry(f"800x{self.HARMONICS_HEIGHT3}")
        ...
        for btn_idx in range(harmony_entries):
            btn_palette = ctk.CTkButton(master=self._frm_harmony_colours,
                                        border_width=3,
                                        width=120,
                                        height=90,
                                        corner_radius=15,
            ...
            self._rendered_harmony_buttons.append(btn_palette)
            ...
            palette_button = self._rendered_harmony_buttons[btn_idx]
            # Add pop-up/context menus to each button...
            menus.append(cbtk.CBtkMenu(palette_button, tearoff=False))
            menus[btn_idx].config(background=self._ctl_frame_high, foreground=self._ctl_text)
            menus[btn_idx].add_command(label="Copy",
                                       command=lambda button_id=btn_idx:
                                       self._copy_harmony_colour(harmony_button_id=button_id))

            btn_palette.bind("<Button-3>",
                             lambda event, menu=menus[btn_idx], button_id=btn_idx: self._context_menu(event, menu))
        if self._keystone_colour is not None:
            self._populate_harmony_colours()
            self.render_keystone_shades_palette(
                keystone_colour=self._btn_keystone_colour.cget('fg_color'),
    ...
    def render_keystone_shades_palette(self, keystone_colour: str, harmony_method: str):
        ...
        for button in self._rendered_keystone_shades:
            button.destroy()
        self._rendered_keystone_shades = []
        ...
        for btn_idx in range(len(shades_list)):
            btn_keystone_shade = ctk.CTkButton(master=self._frm_shades_palette,
            ...
            self._rendered_keystone_shades.append(btn_keystone_shade)

            column += 1

            palette_button = self._rendered_keystone_shades[btn_idx]
            menus.append(cbtk.CBtkMenu(palette_button, tearoff=False))
            menus[btn_idx].config(background=self._ctl_frame_high, foreground=self._ctl_text)
            menus[btn_idx].add_command(label="Copy",
                                       command=lambda button_id=btn_idx:
                                       self._copy_keystone_colour(harmony_button_id=button_id))

            btn_keystone_shade.bind("<Button-3>",
                                    lambda event, menu=menus[btn_idx], button_id=btn_idx: self._context_menu(event,
                                                                                                             menu))
    ...
    def copy_harmonics_to_palette(self):
        harmonic_differential = self.harmony_contrast_differential
        colour_range = [self._btn_keystone_colour.cget('fg_color')]

        for btn_idx in range(len(self._rendered_harmony_buttons)):
            colour = self._rendered_harmony_buttons[btn_idx].cget('fg_color')
            colour_range.append(colour)
        num_harmony_colours = len(colour_range)

        num_theme_palette_tiles = len(self._theme_palette_tiles)
        ...
        for idx in range(num_harmony_colours):
            ...
            self._set_palette_colour(palette_button_id=idx, colour=colour)
        ...
        self._harmony_status_bar.set_status_text(
            status_text=f'Keystone and generated colours copied to palette.')

        self._theme_json_data['provenance']['keystone colour'] = colour_range[0]

        harmony_method = self.opm_harmony_method.get()
        self._theme_json_data['provenance']['harmony method'] = harmony_method
    ...
    def tag_keystone_colour_to_theme(self):
        keystone_colour = self._btn_keystone_colour.cget('fg_color')
        harmony_method = self.opm_harmony_method.get()
        self._theme_json_data['provenance']['keystone colour'] = keystone_colour
        self._theme_json_data['provenance']['harmony method'] = harmony_method
        self._theme_json_data['provenance']['harmony differential'] = self.harmony_contrast_differential
        self._json_state = 'dirty'
        self._set_option_states()
        self._harmony_status_bar.set_status_text(
            status_text=f'Keystone colour tagged to theme {self._theme}.')
    ...
    def lighten_palette_tile(self, palette_button: ctk.CTkButton,
        ...
        if lighter_shade != widget_colour:
            ...
            self._paste_palette_colour(event=None, palette_button_id=palette_button_id)
    ...
    def lighten_widget_property_shade(self, property_widget: ctk.CTkButton,
        ...
        if lighter_shade != widget_colour:
            ...
            self._paste_colour(event=None, widget_property=widget_property)
    ...
    def _copy_harmony_input_colour(self, event=None, shade_copy=False):
        colour = self._btn_keystone_colour.cget('fg_color')
        if shade_copy:
    ...
    def _paste_harmony_keystone_colour(self):
        """Paste the colour currently stored in the paste buffer, to the harmony input button."""
        new_colour = pyperclip.paste()
        if not cbtk.valid_colour(new_colour):
            self._harmony_status_bar.set_status_text(status_text='Attempted paste of non colour code text - ignored.')
            return
        harmony_method = self.opm_harmony_method.get()
    ...
    def set_harmony_keystone(self, colour_code: str, method: str):
        ...
        if colour_code:
            self._btn_keystone_colour.configure(fg_color=colour_code,
                                                hover_color=hover_colour)

        if method:
            # set the harmony method, as tagged in the theme file.
            self.opm_harmony_method.set(method)
            self._keystone_colour = colour_code

    def _copy_keystone_colour(self, event=None, harmony_button_id=None, shade_copy=False):
        colour = self._rendered_keystone_shades[harmony_button_id].cget('fg_color')
        if shade_copy:
            colour = cbtk.contrast_colour(colour, self.shade_adjust_differential)
        pyperclip.copy(colour)
        self._harmony_status_bar.set_status_text(
    ...
    def _copy_harmony_colour(self, event=None, harmony_button_id=None, shade_copy=False):
        colour = self._rendered_harmony_buttons[harmony_button_id].cget('fg_color')
        if shade_copy:
            colour = cbtk.contrast_colour(colour, self.shade_adjust_differential)
        if shade_copy:
            colour = cbtk.contrast_colour(colour, self.shade_adjust_differential)
    ...
    def _harmony_input_colour_picker(self):
        # self.harmonics_label_count
        primary_colour = askcolor(master=self._top_harmony,
                                  initialcolor=self._keystone_colour,
                                  title=f'Harmony Source Colour Selection')

        if primary_colour[1] is not None:
            primary_colour = primary_colour[1]
            self._btn_keystone_colour.configure(fg_color=primary_colour,
                                                hover_color=primary_colour)
            self._status_bar.set_status_text(
                status_text=f'Colour {primary_colour} assigned.')
            self._keystone_colour = primary_colour
            self._populate_harmony_colours()

    def _harmony_colour_picker(self, palette_button_id):
        # self.harmonics_label_count
        primary_colour = askcolor(master=self._top_harmony,
                                  title=f'Harmony Source Colour Selection')
        ...
        if primary_colour[1] is not None:
            primary_colour = primary_colour[1]
            self._rendered_harmony_buttons[0].configure(fg_color=primary_colour,
                                                        hover_color=primary_colour)
            self._status_bar.set_status_text(
    ...
    def _populate_harmony_colours(self):
        primary_colour = self._keystone_colour
        harmony_method = self.opm_harmony_method.get()
        colour_object = ch.Color(hex_to_rgb(self._keystone_colour), "", "")
        ...
        for btn_idx in range(harmony_entries):
            ...
            harmony_colour = rgb_to_hex(colour)
            self._rendered_harmony_buttons[btn_idx].configure(fg_color=harmony_colour,
    ...
    def _load_theme_palette(self, theme_name=None):
        ...
        if theme_name is None:
            theme_name = self._theme_file
        palette_json = theme_name
        self._palette_file = self._palettes_dir / palette_json
        if not self._palette_file.exists():
            self._create_theme_palette(theme_name)

        self._theme_palette = mod.json_dict(json_file_path=self._palette_file)
        ...
        try:
            for entry_id, button in enumerate(self._theme_palette_tiles):
                colour = self._theme_palette[str(entry_id)][mode]
    ...
    def _create_theme(self):
        ...
        if new_theme:
            ...
            if new_theme_path.exists():
                self._status_bar.set_status_text(status_text=f'Theme {new_theme} already exists - '
            ...
            self._theme_json_data['provenance']['date created'] = date_created
            if self._theme_author:
                self._theme_json_data['provenance']['theme author'] = self._theme_author
            else:
                self._theme_json_data['provenance']['theme author'] = 'Unknown'
            self._save_theme()
            self._status_bar.set_status_text(status_text=f'New theme {new_theme} created.')
            self._reload_preview()
    ...
    def _delete_theme(self):
        ...
        confirm = CTkMessagebox(master=self.ctk_control_panel,
                                title='Confirm Action',
                                message=f'All data, for theme "{self._theme}", will be '
                                        f'purged. Are you sure you wish to continue?',
                                options=["Yes", "No"])
        if confirm.get() == 'No':
            self.seg_mode.set(self._appearance_mode)
            return

        os.remove(self._source_json_file)
        source_palette_file = self._theme + '.json'
        source_palette_path = self._palettes_dir / source_palette_file
        ...
        self._opm_properties_filter.configure(state=tk.DISABLED)
        self._btn_save_as.configure(state=tk.DISABLED)
        self._btn_delete.configure(state=tk.DISABLED)
        self._btn_sync_modes.configure(state=tk.DISABLED)
        self._opm_properties_view.configure(state=tk.DISABLED)
        self._swt_render_disabled.configure(state=tk.DISABLED)
        self._btn_reset.configure(state=tk.DISABLED)
        self._btn_save.configure(state=tk.DISABLED)
        self._status_bar.set_status_text(status_text_life=30,
    ...
    def _save_theme_as(self):
        ...
        if new_theme:
            if not valid_theme_name(new_theme):
                self._status_bar.set_status_text(status_text=f'The entered theme name contains prohibited characters '
            ...
            if self._theme_author:
                theme_author = self._theme_author
            else:
                theme_author = 'Unknown'

            if 'provenance' in self._theme_json_data:
                self._theme_json_data['provenance']['theme name'] = new_theme
                self._theme_json_data['provenance']['date created'] = date_saved_as
                ...
                self._theme_json_data['provenance'] = provenance

            # If user has included a ".json" extension, remove it, because we add one below.
            self._theme_file = new_theme + '.json'
            ...
            new_theme_path = self._theme_json_dir / new_theme_json_file
            shutil.copyfile(source_file, new_theme_path)

            new_palette_path = self._palettes_dir / new_theme_json_file
            source_palette_file = self._theme + '.json'
            source_palette_path = self._palette_file
            shutil.copyfile(source_palette_path, new_palette_path)
            self._palette_file = new_palette_path
            self._save_theme()
            self._json_files = self._user_themes_list()
            self.opm_theme.configure(values=self._json_files)
            self._theme = new_theme

    def _paste_colour(self, event, widget_property):
        """Paste the colour currently stored in the paste buffer, to the selected button, where the paste operation
        was invoked."""
        new_colour = pyperclip.paste()
        if not cbtk.valid_colour(new_colour):
            self._status_bar.set_status_text(status_text='Attempted paste of non colour code text - ignored.')
            return
        self._set_widget_colour(widget_property=widget_property, new_colour=new_colour)
    ...
    def _paste_palette_colour(self, event, palette_button_id):
        new_colour = pyperclip.paste()
        if not cbtk.valid_colour(new_colour):
            self._status_bar.set_status_text(status_text='Attempted paste of non colour code - pasted text ignored.')
            return
        self._set_palette_colour(palette_button_id=palette_button_id, colour=new_colour)
        self._status_bar.set_status_text(
            status_text=f'Colour {new_colour} assigned to palette entry {palette_button_id + 1}.')

    def _set_palette_colour(self, palette_button_id, colour):
        hover_colour = cbtk.contrast_colour(colour)
        self._theme_palette_tiles[palette_button_id].configure(fg_color=colour, hover_color=hover_colour)
        self._json_state = 'dirty'
        self._set_option_states()

    def set_filtered_widget_display(self, dummy='dummy'):
        properties_filter = self._opm_properties_filter.get()
    ...
    def _colour_picker(self, palette_button_id):
        prev_colour = self._theme_palette_tiles[palette_button_id].cget('fg_color')
        new_colour = askcolor(master=self._frm_theme_palette,
                              title=f'Pick colour for palette entry number {palette_button_id + 1}',
                              initialcolor=prev_colour)

        if new_colour[1] is not None:
            new_colour = new_colour[1]
            self._theme_palette_tiles[palette_button_id].configure(fg_color=new_colour, hover_color=new_colour)
            self._status_bar.set_status_text(
                status_text=f'Colour {new_colour} assigned to palette entry {palette_button_id + 1}.')
            self._json_state = 'dirty'
            self._set_option_states()
    ...
    @staticmethod
    def _context_menu(event: tk.Event = None, menu: cbtk.CBtkMenu = None):
        menu.tk_popup(event.x_root, event.y_root)

    def _copy_palette_colour(self, event=None, palette_button_id=None, shade_copy=False):
        colour = self._theme_palette_tiles[palette_button_id].cget('fg_color')
        if shade_copy:
            colour = cbtk.contrast_colour(colour, self.shade_adjust_differential)
        pyperclip.copy(colour)
        self._status_bar.set_status_text(
    ...
    def render_widget_properties(self, dummy=None):
        ...
        # self._appearance_mode = self.seg_mode.get()

        widget_frame = self._frm_colour_edit_widgets
        colours = mod.colour_dictionary(self._wip_json)

        view_selection = self._opm_properties_view.get()
        filter_selection = self._opm_properties_filter.get()
        ...
        member_gen = mod.widget_member(widget_entries=rendered_widgets, filter_list=self._filter_list)
        for row in range(1, 30):
            for column_base in range(0, 3):
                ...
                try:
                    ...
                    json_widget_type = mod.json_widget_type(widget_type=widget_type)
                    colour_value = self._theme_json_data[json_widget_type][widget_property]
                ...
                if self._enable_single_click_paste and colour_value != "transparent":
                    self.widgets[key]['widget'].bind("<Button-1>",
                                                     lambda event, wgt_property=key: self._paste_colour(event,
                                                                                                        wgt_property))
                if self._enable_tooltips:
                    if colour_value != 'transparent':
                ...
                if colour_value != "transparent":
                    ...
                    context_menu.add_command(label="Copy",
                                             ...
                                             self._copy_property_colour(event=None, widget_property=wgt_property))

                    context_menu.add_command(label="Paste",
                                             command=lambda wgt_property=key:
                                             self._paste_colour(event=None, widget_property=wgt_property))
                    context_menu.add_separator()
                    shade_up_menu = cbtk.CBtkMenu(context_menu, tearoff=False)
                    shade_up_menu.add_command(label="Lighter", state="normal",
                    ...
                    context_menu.add_command(label="Colour Picker",
                                             command=lambda wgt_property=key:
                                             self.colour_picker(event=None, widget_property=wgt_property))
    ...
    def _sync_appearance_mode(self):
        ...
        if confirm.get() == 'Cancel':
            return
        counter = 0
        for property_id in self._filter_list:
            widget_type, widget_property = mod.widget_property_split(property_id)
            json_widget_type = mod.json_widget_type(widget_type=widget_type)
            if str(self._theme_json_data[json_widget_type][widget_property]) == "transparent":
                continue
            try:
                self._theme_json_data[json_widget_type][widget_property][to_mode] = self.widgets[property_id]["colour"]
                counter += 1
            except TypeError:
                print(f"TypeError on: {widget_type} / {widget_property}")
                print(f"             Current setting: {str(self._theme_json_data[widget_type][widget_property])}")
    ...
    def _sync_theme_palette(self):
        ...
        if self._appearance_mode == 'Light':
            for entry_id, button in enumerate(self._theme_palette_tiles):
                fg_colour = self._theme_palette_tiles[entry_id].cget('fg_color')
                self._theme_palette[str(entry_id)][1] = fg_colour
        else:
            for entry_id, button in enumerate(self._theme_palette_tiles):
                fg_colour = self._theme_palette_tiles[entry_id].cget('fg_color')
                self._theme_palette[str(entry_id)][0] = fg_colour

        self._save_theme_palette()
        self._json_state = 'dirty'
        self._set_option_states()
    ...
    def _reload_preview(self):
        """The _reload_preview method causes a full reload of the preview panel."""
        if self.process:
            self._send_command_json(command_type='program',
                                    command='quit',
                                    parameters=None)
        ...
        if self._tk_render_disabled.get():
            self._send_command_json(command_type='program',
    ...
    def _save_theme(self):
        if 'provenance' in self._theme_json_data:
            ...
            theme_name = self._source_json_file
            theme_name = os.path.splitext(theme_name)[0]
            theme_name = os.path.basename(theme_name)
            self._theme_json_data['provenance']['theme name'] = theme_name
            self._theme_json_data['provenance']['last modified'] = date_modified
            if self._theme_author:
                self._theme_json_data['provenance']['last modified by'] = self._theme_author
            else:
                self._theme_json_data['provenance']['last modified by'] = 'Unknown'
    ...
    def _get_tooltips_setting(self):
        self._enable_tooltips = int(self._tk_enable_tooltips.get())

    def _get_palette_label_setting(self):
        self._enable_palette_labels = int(self._tk_enable_palette_labels.get())

    def _get_last_theme_on_start(self):
        self._last_theme_on_start = int(self._tk_last_theme_on_start.get())

    def _get_single_click_paste_setting(self):
        self._enable_single_click_paste = int(self._tk_enable_single_click_paste.get())
...
class Widget(ABC):
    def __init__(self, widget_type: WidgetType, colours: list):
        ...
        self._light_mode_colour: str = colours[0]
        self._dark_mode_colour: str = colours[1]
...
class CommandController:
    def __init__(self, control_panel: Type[ControlPanel]):
        ...
        self._widgets = list[Widget] = field(default_factory=list)
